...
void TIM_OC1Init(TIM_TypeDef *TIMx, TIM_OCInitTypeDef *TIM_OCInitStruct)
{
    ...
    TIMx->CCER &= (uint16_t)(~(uint16_t)TIM_CC1E);// 比较/捕获使能寄存器
    tmpccer = TIMx->CCER;
    tmpcr2 = TIMx->CTLR2;
    tmpccmrx = TIMx->CHCTLR1;
    tmpccmrx &= (uint16_t)(~((uint16_t)TIM_OC1M)); // [6:4]比较捕获通道1模式设置冻结
    tmpccmrx &= (uint16_t)(~((uint16_t)TIM_CC1S)); // [1:0]比较捕获通道1被配置为输出
